...
class SlackNotifier:
    """Real Slack Web API client (bot token) or webhook, falling back to a
    printed mock when no Slack config is present."""
    ...
    def post_team_message(self, *, text: str) -> tuple[str, str]:
        if config.SLACK_BOT_TOKEN:
            try:
                ...
                print(f"[SLACK] real team message failed, falling back to mock: {exc}")
        if config.SLACK_WEBHOOK_URL:
            try:
                self._post_webhook(text=text)
                return config.SLACK_CHANNEL, "webhook://posted"
            except Exception as exc:
                print(f"[SLACK] webhook team message failed, falling back to mock: {exc}")
    ...
    def post_direct_message(self, *, slack_user_id: str, text: str) -> str:
        ...
        if config.SLACK_BOT_TOKEN:
            try:
                ...
                print(f"[SLACK] real DM failed, falling back to mock: {exc}")
        _, permalink = self._mock_post(channel=f"@{slack_user_id}", text=text)
        return permalink

    def _post_message(self, *, channel: str, text: str) -> dict:
        return self._api_call(endpoint="chat.postMessage", payload={"channel": channel, "text": text})
    ...
    def _api_call(self, *, endpoint: str, payload: dict) -> dict:
        body = parse.urlencode(
            {k: json.dumps(v) if isinstance(v, dict) else v for k, v in payload.items()}
        ).encode("utf-8")
        req = request.Request(
